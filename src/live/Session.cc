...
{
  stream_ = std::make_shared<Stream>(session_name);
...
int32_t Session::SinceStart() const {
...
void Session::CloseUser(UserPtr &user)  {
  ...
  if(!user->destroyed_.exchange(true))
  {
    ...
    if(user->GetUserType() <= UserType::kUserTypePublishWebRtc)
    {
      if(publisher_)
      {
        LIVE_DEBUG << "remove publisher, session_name:" << session_name_
                    << ", user_id:" << user->UserId()
...
void Session::AddPlayer(const PlayerUserPtr &user)  {
  ...
  LIVE_DEBUG << "add player, session_name:" << session_name_
             << ", user_id:" << user->UserId();
